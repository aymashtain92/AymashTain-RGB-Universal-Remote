...
---
## B0. HIGHEST PRIORITY - Handoff consolidation

Do this before anything else. The current hand-off bundle ships ~60
separate files. DeepSeek (and most AIs) cap uploads at 50 files. The
project will only grow, so the file count must shrink now.

Goal: a single-folder hand-off that any AI can accept in one drag.

Rules:

- **No duplicate filenames.** Every file in the bundle must have a
  unique name, even when flattened into one folder.
- **One folder, no nesting.** The user drags one folder, not a tree.
- **Combine all `__init__.py` files** into a single file with
  clearly labelled sections, OR rename each one uniquely (e.g.
  `aymashtain__init__.py`, `tabs__init__.py`,
  `widgets__init__.py`, `ble__init__.py`, `protocol__init__.py`,
  `storage__init__.py`, `audio__init__.py`, `vision__init__.py`,
  `ui__init__.py`). Pick one approach and apply it consistently.
- **Combine all `AI_SYNC_PACK/*.md`** into one file, e.g.
  `AI_SYNC_PACK_COMBINED.md`, with clear `# 00_MASTER_SUMMARY`,
  `# 01_WORKFLOW`, etc. section headers. The next AI reads it top to
  bottom.
- **Combine all `AI_*` bonus files** (`AI_CONTEXT.md`, `AI_NOTES.md`,
  `AI_SESSIONS.md`, `AI_BRIEF.md`, `AI_BACKLOG.md`, `PROJECT.md`)
  into the same combined file, or into a second combined file if the
  first gets too large.
- **Combine the handoff README** into the same file as the sync pack,
  as the very first section.
- **Verify the final count** is under 50 files total. Aim for under
  30 if possible.
- **Update `make_handoff.py`** to build the consolidated structure
  automatically, so every future hand-off is one small folder.

Acceptance:

- [ ] `make_handoff.bat` produces a zip that unzips to a single folder.
- [ ] That folder contains under 50 files.
- [ ] No two files share a name.
- [ ] Every Python file still imports correctly after the
      `__init__.py` renaming (run `python -m pytest -q` and
      `python -m py_compile` on every file after the change).
- [ ] The next AI can find and read the combined sync pack without
      help.
